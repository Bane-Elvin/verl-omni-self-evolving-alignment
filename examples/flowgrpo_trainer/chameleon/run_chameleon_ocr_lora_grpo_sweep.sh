...
SWEEP_TAG=${SWEEP_TAG:-anole_7b_ocr_seen172_$(date +%m%d_%H%M)}
...
REWARD_SERVER_LOG=${REWARD_SERVER_LOG:-$REPO_ROOT/outputs/anole_ocr_grpo/${SWEEP_TAG}_reward_server.log}
...
while read -r train_bsz rollout_n steps; do
    ...
    bash "$SCRIPT_DIR/run_chameleon_ocr_lora_grpo.sh"
done <<< "$SWEEP_SPECS"
